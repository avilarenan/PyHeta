from dataprocessor import run_dataprocessing
...
file_handler = logging.FileHandler(f"../logs/data_worker_{worker_name}.log", mode="a", encoding="utf-8")
...
def do_work(ch, delivery_tag, body):
    ...
    try:
        ...
        for i in tqdm(range(params["repetitions"])): # TODO: make a single repetition be the least acknowledgeable instruction
            ...
            run_dataprocessing(name, params, base_folder, i, R_scripts_folder, credentials_file)
...
def main():
    while True:
        try:
            ...
            channel.basic_consume(queue='dataproc_instructions', on_message_callback=on_message_callback, auto_ack=False)
