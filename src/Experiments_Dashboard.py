...
import pika
import json
from static_info_utils import JSON_CREDENTIALS
import pandas as pd
...
st.set_page_config(
    page_title="Experiments Dashboard",
    page_icon="👋"
)
...
st.markdown("### Launch an ablation experiment")
...
if option_single_or_multi == "Single":
    ...
    st.text(f"Number of experiments: {edited_df.shape[0]}")
...
if st.button("Launch ablation experiment", type="primary"):
    ...
    if exp_pattern["DATASET"] == "FINANCE":
    ...
    elif exp_pattern["DATASET"] == "ENERGY_LOAD":
        ...
        features = [ # Air metrics DATASET
            "temp.c", "wind.mps", "snow.cmsum", "precip.mmsum"
        ]
    else:
    ...
    for ablationid, values in ablations.items():
        ...
        for name, instruction in values.items():
            ...
            channel.basic_publish(
                exchange='',
                routing_key='experiment_instructions',
